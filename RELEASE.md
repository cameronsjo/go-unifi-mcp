...
1. **Edge builds**: The `edge.yml` workflow builds snapshot binaries and pushes
   a `ghcr.io/cameronsjo/go-unifi-mcp:edge` Docker image
2. **Release PR**: Release Please creates or updates a pull request that bumps
   the version and updates `CHANGELOG.md` based on
